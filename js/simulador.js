...
function _simDoOpen(labelU) {
  // OPEN [INPUT|OUTPUT|I-O|EXTEND] FD1 [FD2 ...]
  var modeMatch = labelU.match(/^OPEN\s+(INPUT|OUTPUT|I-O|EXTEND)\s*/i);
  var openMode  = modeMatch ? modeMatch[1].toUpperCase() : 'I-O';
  var rest = labelU.replace(/^OPEN\s+(?:INPUT|OUTPUT|I-O|EXTEND)\s*/i, '');
  ...
  names.forEach(function(fdName) {
    ...
    _simLog('\u2299 OPEN ' + fdName + ' [' + openMode + '] \u2192 arquivo aberto (FS:00)' + fsNote, 'sim-log-info');
  });
  _simRefreshFilesPanel();
}
...
function _simRefreshFilesPanel() {
  ...
  fds.forEach(function(fdName) {
    ...
    var openBadge = isOpen
      ? '<span class="sim-file-open-badge sim-file-open-badge--open" title="Arquivo aberto">\u2299 ABERTO</span>'
...
function _simGetBookFields(bookId) {
  ...
  return book.layout.filter(function(f){ return !f.isGroup; }).map(function(f){ return f.name; });
}
...
function _simAutoImportBookData(fdName) {
  ...
  store.forEach(function(entry) {
    var rec = {};
    activeFields.forEach(function(f) {
      rec[f] = (entry.fields && entry.fields[f] !== undefined) ? entry.fields[f] : '';
    });
    fd.records.push(rec);
  });
